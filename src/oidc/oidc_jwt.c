...
static Json *fetch_json(const char *host, const char *path, const char *what)
{
    ...
    if (!https_get_json(host, path, &st, &body)) {
        oidc_eprintf("oidc_jwt_verify: cannot fetch %s\n", what);
        return NULL;
    }
    if (st != 200) {
        oidc_eprintf("oidc_jwt_verify: %s returned HTTP %d\n", what, st);
        free(body);
        return NULL;
    }
    j = json_parse(body);
...
int oidc_jwt_verify(const char *jwt, const char *aud, const char *iss)
{
    ...
    Json *disc = NULL, *jwks = NULL;
    EVP_MD_CTX *mctx = NULL;
    EVP_PKEY *pkey = NULL;
    uint8_t *sig_raw = NULL;
    const char *alg, *kid;
    const char *dis_iss, *jwks_uri;
    char jwks_host[256] = {0};
    char jwks_path[1024] = {0};
    size_t sig_len = 0;
    int rc = -1;
    ...
    kid = json_get_str(hdr_j, "kid");
    if (!kid) {
        oidc_eprintf("oidc_jwt_verify: JWT header missing kid\n");
        goto out;
    }

    /* claim validation (RFC 7519 / OIDC Core) */
    {
    ...
    /* the issuer's discovery document pins the issuer and names the JWKS
     * endpoint; both are fetched fresh over verified TLS */
    disc = fetch_json(OIDC_AUTH_HOST, "/.well-known/openid-configuration",
    ...
    if (split_https_url(jwks_uri, jwks_host, sizeof jwks_host,
                        jwks_path, sizeof jwks_path) == 0) {
        oidc_eprintf("oidc_jwt_verify: invalid jwks_uri \"%s\"\n",
                     jwks_uri);
        goto out;
    }

    jwks = fetch_json(jwks_host, jwks_path, "JWKS");
    if (!jwks)
        goto out;
    {
        ...
        if (!saw_kid) {
            oidc_eprintf("oidc_jwt_verify: JWKS has no key with kid "
                         "\"%s\"\n", kid);
            goto out;
        }
        if (!pkey) {
    ...
    /* signature over the "header.payload" bytes (RFC 7515 5.1) */
    sig_raw = b64url_decode(sig, &sig_len);

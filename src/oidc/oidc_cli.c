...
static void print_help_footer(void)
{
    printf(
        ...
        "      Also read: SSL_CERT_FILE (non-Windows: the only CA file source chosen by\n"
        "      an environment variable for the HTTPS calls), SSL_CERT_DIR (never read\n"
        "      as a CA directory; before helper exec it is only kept or dropped by\n"
        "      owner/permission - a non-root-owned or group/other-writable path is\n"
        "      unset), HOME, SUDO_USER, SUDO_UID, SUDO_GID,\n"
...
static void print_help_short(void)
{
    printf(
        ...
        "      --socks-listen <SOCKS_LISTEN>  SOCKS5 proxy address (SOCKS mode default: 127.0.0.1:1080; TUN mode: adds an optional SOCKS5+HTTP proxy following TUN routes, off by default)\n"
...
static void print_help_long(void)
{
    printf(
        ...
        "          this is the proxy itself (default: 127.0.0.1:1080);\n"
        "          in TUN mode it adds an optional SOCKS5+HTTP proxy\n"
        "          whose traffic follows the TUN routes (disabled by default)\n"
        "\n"
        "      --socks-mtu <SOCKS_MTU>\n"

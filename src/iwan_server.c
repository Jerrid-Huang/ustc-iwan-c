...
static void usage(const char *prog, FILE *out)
{
    ...
    fprintf(out, "    IWAN_RATE_TOKBAD_MAX=N  per-source per-second budget for\n");
    fprintf(out, "                        known-sid wrong-token DATA frames (the bound\n");
    fprintf(out, "                        class, a live session's peer source);\n");
    fprintf(out, "                        over-budget frames are dropped on a lock-free\n");
    fprintf(out, "                        fast path (default: 4096, 1..65535); a\n");
    fprintf(out, "                        correct-token frame never touches this budget\n");
    fprintf(out, "    IWAN_RATE_CLOSE_MAX=N  per-source per-second budget for\n");
    fprintf(out, "                        known-sid wrong-token CLOSE frames (the bound\n");
    fprintf(out, "                        class); over-budget frames are dropped on a\n");
    fprintf(out, "                        lock-free fast path (default: 4096, 1..65535);\n");
    fprintf(out, "                        a correct-token CLOSE never touches it\n");
    fprintf(out, "    IWAN_SRV_THROTTLE_MS=N  per-session uplink throttle window in\n");
    fprintf(out, "                        milliseconds, applied after a queue-full TUN\n");
    fprintf(out, "                        drop (default: 2, 1..65535)\n");
